...
- `-s` : Filter by canonical circRNA BSJ splice signals (GT–AG / GC–AG / AT–AC) (default: no)
...
### Optional: Using Other RNA Modification Detection Tools

This section describes an optional workflow for applying other RNA modification detection tools to the circRNA data generated by CircRM. This workflow is only applicable to tools that accept Nanopore direct RNA sequencing (DRS) data.

1. **Prepare target reads**  
   After completing **Step 2.1**, the identified `.fast5` files serve as the target circRNA reads for downstream analysis.

2. **Build circRNA reference transcriptome**  
   Use **Step 2.4 (Build circRNA reference FASTA)** to construct the circRNA transcriptome reference. This reference will be used for mapping or annotation in subsequent analyses.

3. **Follow the specific tool workflow**  
   After the above preprocessing steps, proceed with the workflow of the selected RNA modification detection tool as instructed by its documentation.
   
5. **Map modification coordinates to the genome**  
   Apply the logic from the **Methylation_Rate.R** code to convert modification coordinates detected on circRNAs back to genomic coordinates.

> ⚠️ Note: This workflow is intended only for tools that support Nanopore DRS data input.
